...
Regenerate a webhook's token. Requires `manage_server` permission.
...
- `{"cmd": "error", "src": "webhook_regenerate", "val": "Access denied: manage_server permission required"}`
...
- Webhook tokens can only be regenerated by users with the `manage_server` permission.
